...
import { FaCalendarAlt, FaUser, FaArrowLeft, FaTag, FaClock } from "react-icons/fa";
    ...
    <main className="mx-auto w-full max-w-4xl px-4 py-12">
        ...
        <div className="text-[var(--text-secondary)] leading-relaxed">
          {post.content.split('\n\n').map((paragraph, index) => {
            if (paragraph.startsWith('## ')) {
              return (
                <h2 key={index} className="text-2xl font-bold text-[var(--text-primary)] mt-8 mb-4">
                  {paragraph.replace('## ', '')}
                </h2>
              );
            }
            if (paragraph.startsWith('### ')) {
              return (
                <h3 key={index} className="text-xl font-semibold text-[var(--text-primary)] mt-6 mb-3">
                  {paragraph.replace('### ', '')}
                </h3>
              );
            }
      ...
      >
        ...
        <p className="text-[var(--text-secondary)] mb-6">
          Let us help you bring your vision to life with professional video editing services.
        </p>
